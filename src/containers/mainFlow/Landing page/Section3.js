  ...
  return (
    ...
    <Box sx={{ display:'flex', flexDirection:'column', width:'35%', '@media(max-width:1000px)':{width:'40%'},'@media(max-width:800px)':{width:'60%', alignItems:'center'},'@media(max-width:600px)':{width:'90%'}, '@media(max-width:450px)':{width:'100%'}}}>
                <Box sx={{ display:'flex', alignItems:'center'}}>
                    <Typography fontWeight={600} sx={{fontSize:'40px',color:'white','@media(max-width:900px)':{fontSize:'35px'},'@media(max-width:800px)':{fontSize:'30px'}, '@media(max-width:600px)':{fontSize:'25px'},fontFamily:'Raleway' }}>Video Face Swap</Typography>
                    {/* <img src={stick}  alt='stick' /> */}
                </Box>
                <Typography fontSize={16} fontWeight={500} sx={{color:'white',fontFamily:'Raleway'}}>Possibly the most fun of them all, swapping faces on a video is now the new reality! Find your favorite video and go to town.</Typography>
                <Button variant='contained' onClick={() => navigate('/main/videoSwap')} disableElevation sx={{fontWeight:600,backgroundColor:'#FFD600', height:'45px',borderRadius:'12px',width:'fit-content', marginTop:'25px', '&:hover':{backgroundColor:'#FFD600'}}}>Try Video Swap</Button>           
            </Box>
